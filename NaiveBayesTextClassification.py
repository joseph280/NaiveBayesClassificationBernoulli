...
def main_execution_k_folds():
    ...
    with open(path) as f:
        ...
        for x in range(0, folds_number-1):
            ...
            ham.append(ham_new)
        # print('dict_actual: ' + dict_actual + ' prob_spam_vec = ' + prob_spam_vec +' spam mean: '+np.mean(spam) )
        # print('dict_actual dim: ' + len(dict_actual))
        print("^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^")
        ...
        rand_db, db = random_generate_db(db, len_db/folds_number)
        nbcb.naive_bayes_classification_bernoulli_classify(rand_db, dict_actual_new, new2_prob_spam_vec,
                                                           new2_prob_ham_vec, np.mean(spam), np.mean(ham))
        return
